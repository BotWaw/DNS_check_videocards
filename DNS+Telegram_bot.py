...
def check_and_read_config(): # Проверка существования файла конфигурации (с токеном, chatID и ценами). Если файл существует, то предлагается загрузить настройки из файла
    if os.path.exists('config.txt')==True: # Проверка существования файла (ПРОВЕРЕНО)
        answer=''
...
def config_bot_Telegram(): # Настройка и проверка работоспособности бота Telegram  (ПРОВЕРЕНО)
    ...
    while answer!="да" and answer!="Да" and answer!="ДА": # Здесь указано только "да" для того, чтобы цикл каждый раз отправлял сообщение и требовал проверить данные
        ...
        if answer=='да' or answer=='Да' or answer=='ДА':
            ...
            requests.get(f"https://api.telegram.org/bot{token}/sendMessage?chat_id={chat_id}&text=Бот работает") # Эта строка работает, даже если токен и ID в виде строки
...
def config_price(): # Настройка цен на видеокарты, при котором пользователю будет приходить уведомление. Также настроиваем временной интервал обновления цен  (ПРОВЕРЕНО)
    ...
    for cards in all_model:
        ...
        while flag==True: # Цикл для проверки корректности введенных данных, если данные корректны, переходим к следующей карте
            ...
            if price.isdigit() == False or int(price)<=0: # Проверяем не ввели ли пользователь какую-нибудь ерунду
    ...
    while flag == True: # Цикл для проверки корректности введенного временного интервала обновления цен
        ...
        if time_up.isdigit() == False or int(time_up)<60 or int(time_up)>86400:  # Проверяем не ввели ли пользователь какую-нибудь ерунду
...
def save_config(token,chat_id,time_up,all_price_videocards): # Сохранение файла конфигурации, чтобы не вводить каждый раз настройки  (ПРОВЕРЕНО)
    answer=''
    while answer != "да" and answer != "Да" and answer != "ДА" and answer != 'нет' and answer != 'Нет' and answer != 'НЕТ':
...
def check_price(token, chat_id,time_up,all_price_videocards):
    ...
    for a in range(len(all_model)):  # Цикл перебирает модели видеокарт (3090, 3080Ti и т.д.)
        ...
        if a == 0:  # Если браузер еще вообще не открыт, только запускаем процесс проверки
            ...
            driver.switch_to.window(tabs[a])  # Переключаемся на вторую вкладку
            driver.get(URL)
            time.sleep(1)

    while flag == True:  # Цикл, чтобы программа просто работала постоянно
        for a in range(len(all_model)):  # Цикл перебирает модели видеокарт (3090, 3080Ti и т.д.)
            error = True # Нужен для проверки, что инфа о видеокарте собралась ине произошло никакой ошибки или зависания
            ...
            while error==True:
                try:
                    ...
                    availability = driver.find_elements_by_class_name('order-avail-wrap')  # Есть ли в наличии, если да, то когда придет
